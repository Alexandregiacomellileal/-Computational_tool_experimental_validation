...
The algorithm employed to generate the P vectors, involves performing the ATP simulation in mass, obtaining the input-output pairs [**Z**<sub>med</sub> ; **R**<sub>f</sub>], and storaging them in a database $\mathbb{D}_{m\times2n}$ is the [[final_160124.py](https://github.com/Alexandregiacomellileal/Computational_tool_experimental_validation/blob/main/final_160124.py)]. Additionally, for creating the machine learning model to predict the target variables, we utilized [[final_varios_algoritmos.py](https://github.com/Alexandregiacomellileal/Computational_tool_experimental_validation/blob/main/final_varios_algoritmos.py)]. The use of 1000 input-output pairs [**Z**<sub>med</sub> ; **R**<sub>f</sub>] was sufficient to accurate results. Notably, the [[k_parameter_estimator.py](https://github.com/Alexandregiacomellileal/Computational_tool_experimental_validation/blob/main/k_parameter_estimator.py)] algorithm was developed to approximate the value of k, enabling readers to apply the method to different distances $s$ between electrodes. 
